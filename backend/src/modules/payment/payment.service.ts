...
const providerRequest = async <T>(path: string, body: object): Promise<T> => {
  if (!secret()) throw new Error("payment_provider_unavailable");
  ...
  if (!response.ok) throw new Error("payment_provider_rejected");
...
export const buildOrderSnapshot = async (userId: string) => {
  const cart = await CartModel.findOne({ userId }).lean() as any;
  if (!cart?.items?.length) throw new Error("cart_empty");
  const ids = cart.items.map((item: any) => item.productId);
  const products = await ProductModel.find({ _id: { $in: ids }, available: true }).lean() as any[];
  const byId = new Map(products.map(product => [product._id.toString(), product]));
  const items = cart.items.map((item: any) => {
    const product = byId.get(item.productId.toString());
    const quantity = Number(item.quantity);
    if (!product || !Number.isInteger(quantity) || quantity < 1 || quantity > 100) throw new Error("cart_invalid");
    return {
  ...
  const totalAmountPaisa = items.reduce((sum: number, item: any) => sum + item.unitPricePaisa * item.quantity, 0);
  if (!Number.isSafeInteger(totalAmountPaisa) || totalAmountPaisa < 1000) throw new Error("cart_invalid");
  return { items, totalAmountPaisa };
};

const initiateProvider = async (order: any, user: any): Promise<ProviderResult> => {
  if (mockEnabled()) {
  ...
  return providerRequest<ProviderResult>("/epayment/initiate/", {
...
const lookupProvider = async (order: any): Promise<LookupResult> => {
  if (mockEnabled()) {
    if (!String(order.providerPaymentId).startsWith(`mock_${order._id}_`)) throw new Error("invalid_provider_id");
    return {
  ...
  return providerRequest<LookupResult>("/epayment/lookup/", { pidx: order.providerPaymentId });
};
...
export const PaymentService = {
  ...
  async initiate(user: any) {
    const snapshot = await buildOrderSnapshot(user.id);
    const order = await OrderModel.create({ userId: user.id, ...snapshot });
    try {
      const provider = await initiateProvider(order, user);
      if (!provider.pidx || !provider.payment_url) throw new Error("invalid_provider_response");
  ...
  async verify(userId: string, pidx: string) {
    let order = await OrderModel.findOne({ providerPaymentId: pidx, userId });
    if (!order) throw Object.assign(new Error("order_not_found"), { status: 404 });
    if (order.status === "paid") return { order, idempotent: true };
    if (order.status !== "payment_initiated") throw Object.assign(new Error("invalid_order_state"), { status: 409 });

    const provider = await lookupProvider(order);
    if (provider.pidx !== order.providerPaymentId) throw Object.assign(new Error("provider_mismatch"), { status: 400 });
    ...
    try {
      await session.withTransaction(async () => {
        const claimed = await OrderModel.updateOne(
          { _id: order._id, userId, status: "payment_initiated" },
          {
            $set: {
              status: "paid",
        ...
        await CartModel.updateOne(
          { userId },
          { $pull: { items: { productId: { $in: order.items.map((item: any) => item.productId) } } } },
          { session },
        );
      });
    } finally {
    ...
    order = await OrderModel.findOne({ _id: order._id, userId });
    if (!order || order.status !== "paid") throw Object.assign(new Error("payment_finalization_failed"), { status: 500 });
